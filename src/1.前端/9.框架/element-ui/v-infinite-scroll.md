---
title: 无限滚动 v-infinite-scroll指令
author: 李嘉明
...
无限滚动（infinite Scrolling）是一种优化大型列表或长内容的滚动效果的技术。它通过分片渲染可见区域内的元素，而不是一上来直接渲染整个列表，通过不断下滑分片的形式的方式去渲染内容 。来提高性能和内存效率。

<!-- 无限滚动（Virtual Scrolling）是一种优化大型列表或长内容的滚动效果的技术。它通过只渲染可见区域内的元素，而不是渲染整个列表，来提高性能和内存效率。 -->

## v-infinite-scroll

无限滚动 ，在我们移动端开发中的就是常客了。就不在使用 分页来加载我们的内容，只能是用户不断向上滑动，加载更多内容，这样使得移动端的体验得到许多提升
今天，我们要介绍的就是 `EL` 中 的 无限滚动指令

<!-- 无限滚动 ，在我们移动端开发中的就是常客了。当页面渲染过多时，我们就急需一种，**只渲染 可见区域内容的解决方案** 帮助我们去解决性能瓶颈的问题。今天，我们要介绍的就是 `EL` 中 的 无限滚动指令 -->

**需要了解的无限滚动实现原理**

实现的原理就类似于 针对 大数据量的加载，比如以下几种方式, **但是以下几种方式，还是会直接渲染全部，仅仅是将渲染放缓了，不会去影响主线程的执行，也不会造成主线程卡顿的** 那这样，把其 放在 `Web Worker` 中岂不是更好，当然我们无限滚动肯定不是这样的, **而是只渲染可见区域下的元素，当用户主动拉取更多数据时，从而 不断的 在之前的基础上 增加元素内容。这样我们的元素节点会不断的增加，直到不再需要渲染数据了**
...
最大的不同点： 无限滚动 的DOM 节点 是不断 递增的 。而 虚拟滚动，永远只会渲染可见区域的内容,DOM结构 是 复用的
